...
public final class AppSecurity {
    ...
    /** PIN protection is opt-in: the field team can enable it from Settings instead of being forced through setup on first launch. */
    public boolean isEnabled(){return preferences.getBoolean("pin_enabled",false);}
    public void setEnabled(boolean enabled){preferences.edit().putBoolean("pin_enabled",enabled).apply();}
    public void disableAndClear(){preferences.edit().putBoolean("pin_enabled",false).putString("pin_hash","").putString("pin_salt","").putInt("failed_attempts",0).putLong("lockout_until",0).apply();}
