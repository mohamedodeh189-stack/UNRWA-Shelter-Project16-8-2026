...
public final class CadFileProvider extends ContentProvider {
    ...
    @Override public String getType(Uri uri){String name=String.valueOf(uri.getLastPathSegment()).toLowerCase();return name.endsWith(".dwg")?"application/vnd.dwg":name.endsWith(".dxf")?"application/dxf":name.endsWith(".pdf")?"application/pdf":name.endsWith(".xlsx")?"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":name.endsWith(".ttf")?"font/ttf":"application/octet-stream";}
