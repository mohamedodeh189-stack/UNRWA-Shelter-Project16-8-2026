...
public final class PdfMapViewer {
    ...
    private static final class Session {
        ...
        Session(Activity activity,int rawResource,int initialPage,String title)throws Exception{
            ...
            TextView hint=label("كبّر بإصبعين • اسحب للتحرك • زر الضبط يعيد الصفحة",11,MUTED,true);hint.setGravity(Gravity.CENTER);hint.setBackgroundColor(0xddffffff);FrameLayout.LayoutParams hintParams=new FrameLayout.LayoutParams(-1,dp(34),Gravity.BOTTOM);stage.addView(hint,hintParams);
        ...
        private void load(int requested){if(renderer==null)return;pageIndex=Math.max(0,Math.min(requested,renderer.getPageCount()-1));try{
                PdfRenderer.Page page=renderer.openPage(pageIndex);Bitmap nextBitmap;try{int screen=activity.getResources().getDisplayMetrics().widthPixels;int targetWidth=Math.min(2400,Math.max(1400,screen*3));nextBitmap=render(page,targetWidth);}
                catch(OutOfMemoryError lowMemory){nextBitmap=render(page,1200);}finally{page.close();}
                image.setImageBitmap(null);if(bitmap!=null&&!bitmap.isRecycled())bitmap.recycle();bitmap=nextBitmap;image.setImageBitmap(bitmap);image.resetZoom();
