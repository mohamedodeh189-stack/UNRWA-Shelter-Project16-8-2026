...
public final class AddressClassifier {
    ...
    private static final class Entry {
        ...
        Entry(String v, String s, String st, int si, int sti) {
            variant = v; sector = s; street = st; sectorIndex = si; streetIndex = sti;
        }
    }
    ...
    public AddressClassifier(Context context) throws Exception {
        ...
        for (int si = 0; si < SECTOR_ORDER.size(); si++) {
            ...
            for (int sti = 0; sti < streets.size(); sti++) {
                ...
                for (String variant : variants(street)) entries.add(new Entry(variant, sector, street, si, sti));
            }
        }
    }
    ...
    private static Set<String> variants(String street) {
    ...
    public Match classify(String address) {
        ...
        if (best != null && bestScore >= .78) {
            ...
            result.review = "تم تحديد القطاع فقط — الشارع بحاجة لمراجعة";
        } else {
            bestScore = 0; result.review = "غير موجود في المراجع — بحاجة لمراجعة";
        }
