...
OUTPUT = ROOT / "output" / "pdf" / "Yarmouk_Camp_Field_User_Guide_AR_v1.5.6.pdf"
...
def build() -> None:
    ...
    c.setFont("Arabic", 12)
    c.setFillColor(white)
    c.drawRightString(WIDTH - MARGIN, HEIGHT - 300, rtl("الإصدار 1.5.6 - يعمل دون إنترنت"))
    ...
    y = card(c, y, "المخطط الكامل والقطاعات", "المخطط الكامل للنظرة العامة لأن أسماء مصدره مرسومة كأشكال CAD سميكة. لقراءة أسماء الشوارع بوضوح افتح صفحة القطاع من الحدود الملونة؛ ويظهر اسم القطاع في الرأس وفي شارة ثابتة فوق المخطط.", accent=BLUE, height=104)
    y = card(c, y, "تثبيت NART في FastView - خطوة بخطوة", "1) من التطبيق اضغط «شرح عربي وحفظ خط NART». 2) احفظ NART.TTF في التنزيلات. 3) افتح FastView ثم القائمة ← الإعدادات (Settings) ← الخطوط (Fonts). 4) اضغط + واختر NART.TTF. 5) أغلق DWG أو DXF وافتحه من جديد.", accent=TEAL, height=124)
    ...
    y = card(c, y, "خصوصية الشاشة والنسخ", "يسمح التطبيق بلقطات الشاشة عند الحاجة؛ احفظ أي لقطة تحتوي بيانات شخصية في مكان آمن. يبقى النسخ السحابي التلقائي لقاعدة البيانات معطلاً، ويعمل التطبيق دون صلاحية إنترنت.", accent=PURPLE)
    ...
    y2 -= 8
    draw_rtl(c, "الإصدار 1.5.6 - مشروع المأوى في مخيم اليرموك - آب 2026", WIDTH - MARGIN - 22, y2, WIDTH - 2 * MARGIN - 44, size=10, color=MUTED)
    c.save()
