...
OUTPUT = ROOT / "output" / "pdf" / "Yarmouk_Camp_Field_User_Guide_AR_v1.5.0.pdf"
...
def build() -> None:
    ...
    c.setFont("Arabic", 12)
    c.setFillColor(white)
    c.drawRightString(WIDTH - MARGIN, HEIGHT - 300, rtl("الإصدار 1.5.0 - يعمل دون إنترنت"))
    ...
    c.setFillColor(HexColor("#D7EDF5"))
    c.drawRightString(WIDTH - MARGIN, 112, rtl("شكر وتقدير للمهندس عبد المالك أبو حرب والمهندس محمد عبد العال"))
    c.setFont("Arabic", 8.5)
    ...
    y = card(c, y, "تعبئة جدول الكميات", "اختر الكميات المطلوبة من البنود المرجعية الـ37. يحسب التطبيق التكلفة والإجمالي والدفعتين آلياً.", accent=GOLD, number="3")
    ...
    y = card(c, y, "أدخل الكمية فقط", "يعرض التطبيق اسم البند والوحدة والسعر المرجعي. اضغط اسم البند لقراءة الوصف الكامل وسبب التدخل، ثم اكتب الكمية المطلوبة.", accent=GOLD)
    ...
    y -= 125
    y = card(c, y, "الإجمالي والدفعات", "الإجمالي هو مجموع تكاليف البنود. الدفعة الأولى = 60% من الإجمالي، والدفعة الثانية = 40% من الإجمالي.", accent=TEAL)
    ...
    y = card(c, y, "عرض PDF داخل التطبيق", "الخيار المضمون دون برامج إضافية. يدعم التكبير والسحب، وجرى تضمين الخطوط وقص هوامش صفحات القطاعات لتحسين الوضوح.", accent=BLUE)
    y = card(c, y, "فتح CAD", "يفتح المخطط الكامل بصيغة DWG والمخطط المقسّم بصيغة DXF في تطبيق CAD مثبت على الهاتف. إذا لم يوجد عارض متوافق، استخدم PDF.", accent=TEAL)
    ...
    y = card(c, y, "نسخة JSON", "من الإعدادات اختر «نسخة احتياطية JSON». تتضمن المستفيدين والعناوين والمراحل والحالات وجدول الكميات وفهرس الصور.", accent=TEAL)
    y = card(c, y, "مجلد الصور", "انسخ مجلد الصور دورياً إلى مكان آمن. ملف JSON يحفظ مسارات الصور وملاحظاتها لكنه لا يضم ملفات الصور نفسها.", accent=BLUE)
    y = card(c, y, "العمل دون إنترنت", "قاعدة البيانات والمخططات ومرجع الكميات ودليل الاستخدام مرفقة داخل التطبيق. لا تحتاج الوظائف الميدانية الأساسية إلى اتصال.", accent=PURPLE)
    ...
    y2 -= 8
    y2 = draw_rtl(c, "شكر وتقدير للمهندس عبد المالك أبو حرب والمهندس محمد عبد العال على دعمهما وتعاونهما في تطوير العمل الميداني.", WIDTH - MARGIN - 22, y2, WIDTH - 2 * MARGIN - 44, size=11, color=TEXT, leading=18)
    y2 -= 8
    draw_rtl(c, "الإصدار 1.5.0 - مشروع المأوى في مخيم اليرموك - آب 2026", WIDTH - MARGIN - 22, y2, WIDTH - 2 * MARGIN - 44, size=10, color=MUTED)
    c.save()
