...
public final class PdfMapViewer {
    ...
    private static final class Session {
        ...
        private final TextView titleView,pageView,mapBadge;private final Button previous,next;private ParcelFileDescriptor descriptor;private PdfRenderer renderer;private Bitmap bitmap;private int pageIndex;
        ...
        Session(Activity activity,int rawResource,int initialPage,String title)throws Exception{
            ...
            LinearLayout header=new LinearLayout(activity);header.setGravity(Gravity.CENTER_VERTICAL);header.setPadding(dp(12),dp(8),dp(12),dp(8));header.setBackgroundColor(NAVY);root.addView(header,new LinearLayout.LayoutParams(-1,dp(92)));
            Button close=button("إغلاق",0xffBD4868);close.setOnClickListener(v->dialog.dismiss());header.addView(close,new LinearLayout.LayoutParams(dp(86),dp(52)));
            titleView=label(title,17,Color.WHITE,true);titleView.setGravity(Gravity.CENTER);titleView.setMaxLines(2);titleView.setLineSpacing(dp(2),1f);header.addView(titleView,new LinearLayout.LayoutParams(0,-1,1));
            ...
            mapBadge=label(title,15,Color.WHITE,true);mapBadge.setGravity(Gravity.CENTER);mapBadge.setMaxLines(2);mapBadge.setBackgroundColor(0xee0E7490);FrameLayout.LayoutParams badgeParams=new FrameLayout.LayoutParams(-2,dp(48),Gravity.TOP|Gravity.CENTER_HORIZONTAL);badgeParams.setMargins(dp(14),dp(10),dp(14),0);stage.addView(mapBadge,badgeParams);
            TextView hint=label("تكبير موسّع بإصبعين • اسحب للتحرك • زر ضبط يعيد الصفحة كاملة",13,MUTED,true);hint.setGravity(Gravity.CENTER);hint.setBackgroundColor(0xddffffff);FrameLayout.LayoutParams hintParams=new FrameLayout.LayoutParams(-1,dp(40),Gravity.BOTTOM);stage.addView(hint,hintParams);
            ...
            pageView=label("",15,NAVY,true);pageView.setGravity(Gravity.CENTER);controls.addView(pageView,new LinearLayout.LayoutParams(dp(100),dp(48)));
            ...
            root.setOnApplyWindowInsetsListener((view,insets)->{int top,bottom,left,right;if(android.os.Build.VERSION.SDK_INT>=30){int types=android.view.WindowInsets.Type.systemBars()|android.view.WindowInsets.Type.displayCutout();android.graphics.Insets safe=insets.getInsets(types);top=safe.top;bottom=safe.bottom;left=safe.left;right=safe.right;}else{top=Math.max(insets.getStableInsetTop(),insets.getSystemWindowInsetTop());bottom=Math.max(insets.getStableInsetBottom(),insets.getSystemWindowInsetBottom());left=Math.max(insets.getStableInsetLeft(),insets.getSystemWindowInsetLeft());right=Math.max(insets.getStableInsetRight(),insets.getSystemWindowInsetRight());}header.setPadding(left+dp(12),top+dp(8),right+dp(12),dp(8));ViewGroup.LayoutParams hp=header.getLayoutParams();hp.height=top+dp(92);header.setLayoutParams(hp);controls.setPadding(left+dp(8),dp(7),right+dp(8),bottom+dp(7));ViewGroup.LayoutParams cp=controls.getLayoutParams();cp.height=bottom+dp(68);controls.setLayoutParams(cp);return insets;});root.requestApplyInsets();
        ...
        private void load(int requested){if(renderer==null)return;pageIndex=Math.max(0,Math.min(requested,renderer.getPageCount()-1));try{
                ...
                int count=renderer.getPageCount();pageView.setText("صفحة "+(pageIndex+1)+" / "+count);String visibleTitle=baseTitle+(count>1?" - صفحة "+(pageIndex+1):"");titleView.setText(visibleTitle);mapBadge.setText(visibleTitle);previous.setEnabled(pageIndex>0);next.setEnabled(pageIndex<count-1);previous.setAlpha(previous.isEnabled()?1f:.35f);next.setAlpha(next.isEnabled()?1f:.35f);
        ...
        private int qualityWidth(PdfRenderer.Page page){int screen=activity.getResources().getDisplayMetrics().widthPixels;ActivityManager manager=(ActivityManager)activity.getSystemService(Context.ACTIVITY_SERVICE);int memoryClass=manager==null?192:manager.getMemoryClass();long pixelBudget=memoryClass>=384?20_000_000L:memoryClass>=256?16_000_000L:memoryClass>=192?12_000_000L:8_000_000L;float ratio=page.getHeight()/(float)page.getWidth();int budgetWidth=(int)Math.sqrt(pixelBudget/Math.max(.2f,ratio));int desired=Math.min(4800,Math.max(2800,screen*7));return Math.max(2000,Math.min(desired,budgetWidth));}
        ...
        private Button button(String text,int color){Button value=new Button(activity);value.setText(text);value.setTextColor(Color.WHITE);value.setTextSize(14);value.setAllCaps(false);value.setTypeface(null,Typeface.BOLD);value.setBackgroundColor(color);return value;}
