...
- Version: `1.5.7` (`versionCode 157`)
- APK: `dist/Yarmouk_Camp_Field_Android_v1.5.7.apk`
- SHA-256: `28FC0530A5BCEEE8B540F3D48F8E1A75AC48F3598E725836E5800604CDC9D39B`
...
### V1.5.7 clear full-camp PDF replacement

- Replaced the full-camp source with the newly supplied 6.3 MB PDF whose Arabic street names remain legible at high zoom.
- Regenerated the cropped Android PDF and the interactive overview bitmap from that source; APK content was hash/pixel verified.
- Archived the previous full-map source under `source_maps/archive/`.

### V1.5.6 visible sector title in PDF viewer

- Increased the PDF viewer header height and allowed the title to occupy two lines without clipping beneath system bars.
- Added a fixed high-contrast badge over the map stage showing the sector name and page number during zoom and pan.

### V1.5.5 corrected sector geometry

- Replaced the approximate horizontal envelopes with shared, irregular sector boundaries retraced from the user's green markup over the full camp map.
- Moved the seven compact labels to the corrected sector centers while preserving the translucent fills and colored borders.
