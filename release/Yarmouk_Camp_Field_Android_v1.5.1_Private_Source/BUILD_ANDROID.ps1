...
& (Join-Path $BuildTools 'aapt2.exe') link -o $UnsignedApk -I $Platform --manifest (Join-Path $ProjectRoot 'app\src\main\AndroidManifest.xml') -A (Join-Path $ProjectRoot 'app\src\main\assets') --java $GeneratedDir --auto-add-overlay --min-sdk-version 24 --target-sdk-version 36 --version-code 151 --version-name '1.5.1' $CompiledRes
...
$FinalApk = Join-Path $DistDir 'Yarmouk_Camp_Field_Android_v1.5.1.apk'
...
try {
    foreach ($requiredEntry in @('classes.dex', 'assets/yarmouk_addresses.json', 'assets/boq_reference.json', 'assets/yarmouk_camp_map.dwg', 'assets/yarmouk_sector_maps.dxf', 'res/drawable-nodpi-v4/unrwa_logo.gif', 'res/raw/yarmouk_camp_map.pdf', 'res/raw/yarmouk_sector_maps.pdf', 'res/raw/user_guide_ar.pdf')) {
