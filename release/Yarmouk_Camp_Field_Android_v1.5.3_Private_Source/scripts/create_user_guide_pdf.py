...
OUTPUT = ROOT / "output" / "pdf" / "Yarmouk_Camp_Field_User_Guide_AR_v1.5.3.pdf"
...
def build() -> None:
    ...
    c.setFont("Arabic", 12)
    c.setFillColor(white)
    c.drawRightString(WIDTH - MARGIN, HEIGHT - 300, rtl("الإصدار 1.5.3 - يعمل دون إنترنت"))
    ...
    y = card(c, y, "عرض PDF داخل التطبيق", "الخيار المضمون للخط العربي. يدعم تكبيراً موسعاً بإصبعين والسحب، مع رندر أعلى دقة حسب ذاكرة الهاتف وزر «ضبط» للعودة إلى الصفحة كاملة.", accent=BLUE)
    y = card(c, y, "CAD وخط NART", "إذا ظهر الخط مشوهاً في FastView، اضغط «حفظ خط NART»، ثم في FastView افتح Settings ← Fonts ← + واختر NART.TTF، وأعد فتح DWG أو DXF. استخدم PDF إذا بقي خط آخر ناقصاً.", accent=TEAL, height=104)
    ...
    y2 -= 8
    draw_rtl(c, "الإصدار 1.5.3 - مشروع المأوى في مخيم اليرموك - آب 2026", WIDTH - MARGIN - 22, y2, WIDTH - 2 * MARGIN - 44, size=10, color=MUTED)
    c.save()
