...
public final class SpreadsheetImporter {
    ...
    private static List<String> readSharedStrings(ZipFile zip)throws Exception{
        ...
        XMLReader parser=XMLReaderFactory.createXMLReader();parser.setContentHandler(new DefaultHandler(){StringBuilder text=new StringBuilder();StringBuilder current=new StringBuilder();boolean inT;
    ...
    private static List<SheetRow> parseSheet(InputStream stream,List<String>shared)throws Exception{
        List<SheetRow> rows=new ArrayList<>();XMLReader parser=XMLReaderFactory.createXMLReader();
    ...
    private static Result fromJson(Context context,Uri uri)throws Exception{
        ...
        if(root instanceof JSONArray)array=(JSONArray)root;else{JSONObject o=(JSONObject)root;array=o.optJSONArray("beneficiaries");if(array==null)array=o.optJSONArray("rows");if(array==null)throw new IllegalArgumentException("لا توجد قائمة beneficiaries أو rows في JSON");}
        Result result=new Result();for(int i=0;i<array.length();i++){JSONObject o=array.getJSONObject(i);AppDatabase.Beneficiary b=new AppDatabase.Beneficiary();
            b.sequence=i+1;b.name=first(o,"name","master_name","اسم المستفيد","الاسم");b.registration=first(o,"registration","reg_number","رقم التسجيل");
            b.relation=first(o,"relation","صلة القرابة");b.relatedTo=first(o,"related_to","main_owner","مع من");b.phone1=phone(first(o,"phone1","phone","الهاتف"));
            ...
            b.visitStatus=first(o,"visit_status");if(b.visitStatus.isEmpty())b.visitStatus="pending";b.notes=first(o,"notes","ملاحظات");if(!b.name.isEmpty()||!b.address.isEmpty())result.rows.add(b);}return result;
    }
    ...
    private static Result convert(List<SheetRow>matrix){Result result=new Result();if(matrix.isEmpty())return result;int header=-1;Map<String,Integer>cols=null;int best=-1;
        ...
        for(int r=header+1;r<matrix.size();r++){SheetRow row=matrix.get(r);AppDatabase.Beneficiary b=new AppDatabase.Beneficiary();b.sequence=r-header;
            b.name=get(row,cols,"name");b.registration=get(row,cols,"registration");b.relation=get(row,cols,"relation");b.relatedTo=get(row,cols,"related_to");
    ...
    private static Map<String,Integer>columns(SheetRow row){Map<String,Integer>cols=new LinkedHashMap<>();for(Map.Entry<Integer,String>cell:row.cells.entrySet()){String t=AddressClassifier.normalize(cell.getValue());if(t.isEmpty())continue;
        ...
        if(t.contains("مع من")||t.contains("مرتبط")||t.contains("اسم رب الاسره")||t.equals("related to")||t.equals("related person")||t.equals("main owner"))put(cols,"related_to",cell.getKey());
