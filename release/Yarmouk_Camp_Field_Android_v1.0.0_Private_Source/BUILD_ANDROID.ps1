...
$PlatformSource = Join-Path $SdkRoot 'platforms\android-37.0\android.jar'
...
$StorePassword = 'YarmoukField2026!'
$Alias = 'yarmouk-field'
...
& (Join-Path $BuildTools 'aapt2.exe') link -o $UnsignedApk -I $Platform --manifest (Join-Path $ProjectRoot 'app\src\main\AndroidManifest.xml') -A (Join-Path $ProjectRoot 'app\src\main\assets') --java $GeneratedDir --auto-add-overlay --min-sdk-version 24 --target-sdk-version 36 --version-code 100 --version-name '1.0.0' $CompiledRes
...
$FinalApk = Join-Path $DistDir 'Yarmouk_Camp_Field_Android_v1.0.0.apk'
...
try {
    foreach ($requiredEntry in @('classes.dex', 'assets/yarmouk_addresses.json', 'res/drawable-nodpi-v4/unrwa_logo.gif')) {
