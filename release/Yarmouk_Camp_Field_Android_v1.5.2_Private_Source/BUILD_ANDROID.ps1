...
& (Join-Path $BuildTools 'aapt2.exe') link -o $UnsignedApk -I $Platform --manifest (Join-Path $ProjectRoot 'app\src\main\AndroidManifest.xml') -A (Join-Path $ProjectRoot 'app\src\main\assets') --java $GeneratedDir --auto-add-overlay --min-sdk-version 24 --target-sdk-version 36 --version-code 152 --version-name '1.5.2' $CompiledRes
...
$FinalApk = Join-Path $DistDir 'Yarmouk_Camp_Field_Android_v1.5.2.apk'
